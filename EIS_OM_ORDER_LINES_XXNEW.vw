CREATE OR REPLACE FORCE VIEW EIS_OM_ORDER_LINES_XXNEW
(
   ORDER_NUMBER,
   LINE_NUMBER,
   ORDERED_ITEM,
   ...
   POB2B_PO_VENDOR_SITE_CODE,
   POB2B_PO_APPROVED_DATE,
...
AS
   SELECT --In Addition to std. EIS table joins, this view has joins to following tables:
            ...
            CASE
               WHEN ombill.current_billing_date IS NOT NULL
               THEN
                  ombill.current_billing_date
               ELSE
                  ombill.TRX_DATE
            END
               ombill_current_billing_date,           /* MGRC customization */
            ...
            /*ombill.ib_fulfill ombill_ib_fulfill,*/
            NULL ombill_ib_fulfill,
            ...
            pob2b.pob2b_po_vendor_site_code,          /* MGRC customization */
            pob2b.pob2b_po_approved_date,             /* MGRC customization */
            ...
            pob2b.pob2b_invoice_id,                   /* MGRC customization */
		   ...
		   oe_transaction_types_vl oltt,        /* MGRC customization */ -- Added by IT Convergence as Per R12.2.10 Upgrade 
            ...
            (                                                    ----1,031,623
                                                ...
                                                --term.name last_payment_term,
                                                 --billing_cyc.cycle_frequency
                                                                        --from
                                        --bolinf.xxmgrc_om_billing_lines bill,
                                                         --apps.ra_terms term,
                                        --ar.ar_cons_bill_cycles_b billing_cyc
                           --where bill.last_payment_term_id = term.term_id(+)
            --and bill.last_billing_cycle_id = billing_cyc.billing_cycle_id(+)
                                                                   --1,031,623
             SELECT                                      /*+ NO_CPU_COSTING */
                   bill.*,
                      ar_inv.trx_date,
                      term.name last_payment_term,
                      billing_cyc.cycle_frequency
               FROM   --bolinf.xxmgrc_om_billing_lines bill, -- Commented by IT Convergence as Per R12.2.10 Upgrade 
			   xxmgrc_om_billing_lines bill, -- Added by IT Convergence as Per R12.2.10 Upgrade 
                      (  SELECT                          /*+ NO_CPU_COSTING */
                               MAX (ra_customer_trx_all.trx_date) trx_date,
                                  --ra_customer_trx_lines_all.sales_order order_num,
                       ...
                       --group by ra_customer_trx_lines_all.sales_order, ra_customer_trx_lines_all.interface_line_attribute6
                       GROUP BY   ra_customer_trx_lines_all.interface_line_attribute6)
                      ar_inv,
                     /*  apps.ra_terms term,  -- commented by IT Convergence as Per R12.2.10 Upgrade  
                      ar.ar_cons_bill_cycles_b billing_cyc */ -- commented by IT Convergence as Per R12.2.10 Upgrade 
					   ra_terms term, -- Added by IT Convergence as Per R12.2.10 Upgrade 
                      ar_cons_bill_cycles_b billing_cyc -- Added by IT Convergence as Per R12.2.10 Upgrade 
              WHERE   bill.oe_line_id = ar_inv.oe_line_id(+)
                      AND bill.last_payment_term_id = term.term_id(+)
            ...
            (SELECT   *
               FROM   (SELECT   ROW_NUMBER ()
                                   OVER (
                                      PARTITION BY xpbt.link_to_order_line_id
                                      ORDER BY xpbt.last_update_date DESC
                                   )
                                   pob2b_trx_row_num,
                                pha.po_header_id pob2b_po_header_id,
                                pha.segment1 pob2b_po_number,
                                pla.po_line_id pob2b_po_line_id,
                                pla.line_num pob2b_po_line_num,
                                xpbt.link_to_order_number pob2b_order_num,
                                xpbt.link_to_order_header_id
                                   pob2b_order_header_id,
                                xpbt.link_to_order_line_id
                                   pob2b_order_line_id,
                                xpbt.order_line_number pob2b_order_line_num,
                                pha.closed_code pob2b_po_header_status_code,
                                pha.authorization_status
                                   pob2b_authorization_status,
                                xpbt.buyer pob2b_buyer,
                                pha.vendor_id pob2b_vendor_id,
                                pha.vendor_site_id pob2b_vendor_site_id,
                                pha.vendor_contact_id pob2b_vendor_contact_id,
                                xpbt.comments pob2b_comments,
                                xpbt.header_notes pob2b_header_notes,
                                xpbt.po_cancel_ok pob2b_po_cancel_ok,
                                xpbt.cancel_header pob2b_cancel_header,
                                xpbt.line_action pob2b_line_action,
                                pla.closed_code pob2b_po_line_status_code,
                                xpbt.po_line_cancel_ok
                                   pob2b_po_line_cancel_ok,
                                xpbt.b2b_upd_ok pob2b_b2b_upd_ok,
                                xpbt.b2b_rcv_ok pob2b_b2b_rcv_ok,
                                xpbt.need_by_date pob2b_need_by_date,
                                xpbt.unit_price pob2b_unit_price,
                                xpbt.oe_po_price pob2b_oe_po_price,
                                CASE
                                   WHEN pla.quantity <=
                                           po_ln_loc.qty_received
                                   THEN
                                      0
                                   ELSE
                                      pla.quantity - po_ln_loc.qty_received
                                END
                                   pob2b_qty_open,
                                po_ln_loc.qty_received pob2b_qty_received,
                                po_ln_loc.qty_cancelled pob2b_qty_cancelled,
                                po_ln_loc.qty_billed pob2b_qty_billed,
                                xpbt.last_update_date pob2b_last_update_date,
                                xpbt.last_updated_by pob2b_last_updated_by,
                                /* added on 11/29/2018 - begins */
                                pha.creation_date pob2b_po_creation_date,
                                pla.quantity pob2b_po_qty,
                                pla.unit_price pob2b_po_unit_price,
                                aps.vendor_name pob2b_po_vendor_name,
                                apssa.vendor_site_code
                                   pob2b_po_vendor_site_code,
                                po_act.action_date pob2b_po_approved_date,
                                po_act.approved_by pob2b_po_approved_by,
                                po_rcpt.transaction_date
                                   pob2b_po_received_date,
                                po_rcpt.received_by pob2b_po_received_by,
                                party_cont.party_name
                                   pob2b_vendor_contact_name,
                                ap_inv.ap_invoice_id pob2b_invoice_id,
                                ap_inv.ap_inv_number pob2b_inv_number,
                                ap_inv.ap_inv_payment_amount
                                   pob2b_inv_payment_amount,
                                ap_inv.ap_inv_last_payment_date
                                   pob2b_inv_last_payment_date,
                                gcc.segment2 pob2b_po_dist_gl_accnt
                         /* added on 11/29/2018 - end */
                         FROM   po_headers_all pha,
                                ...
                                (SELECT   *
                                   FROM   (SELECT   pt.*,
                                                    ROW_NUMBER ()
                                                       OVER (
                                                          PARTITION BY pt.po_header_id,
                                                                       pt.po_line_id
                                                          ORDER BY
                                                             pt.last_update_date DESC,
                                                             CASE
                                                                WHEN pt.link_to_order_line_id IS NULL
                                                                THEN
                                                                   0
                                                                ELSE
                                                                   1
                                                       ...
                                                       rown
                                             FROM  -- bolinf.xxmgrc_pob2b_trxs pt -- Commented by IT Convergence as Per R12.2.10 Upgrade
											 xxmgrc_pob2b_trxs pt -- Added by IT Convergence as Per R12.2.10 Upgrade
											 )
                                  WHERE   rown = 1) xpbt,
                                ap_suppliers aps,
                              /*   ap.ap_supplier_sites_all apssa, -- Commented by IT Convergence as Per R12.2.10 Upgrade Start
                                po.po_distributions_all pda,
                                gl.gl_code_combinations gcc, */ -- Commented by IT Convergence as Per R12.2.10 UpgradeEnd
								  ap_supplier_sites_all apssa, -- Added by IT Convergence as Per R12.2.10 Upgrade Start
                                po_distributions_all pda,
                                gl_code_combinations gcc, -- Added by IT Convergence as Per R12.2.10 Upgrade End
                                (SELECT   *
                                   FROM   (SELECT   pah.object_id,
                                            ...
                                            WHERE   pah.action_code =
                                                       'APPROVE'
                                                    AND pah.object_type_code =
                                                          'PO'
                                                    AND pah.employee_id =
                                                          papf.person_id
                                                    AND (pah.action_date BETWEEN papf.effective_start_date
                                                                             AND  papf.effective_end_date))
                                  WHERE   latest_rec = 1) po_act,
                                (SELECT   *
                                   FROM   (SELECT   rt.po_header_id,
                                                    rt.po_line_id,
                                                    rt.po_revision_num,
                                                    rt.transaction_date,
                                                    fu.user_name received_by,
                                                    ROW_NUMBER ()
                                                       OVER (
                                                          PARTITION BY rt.po_header_id,
                                                                       rt.po_line_id,
                                                                       rt.po_revision_num
                                                          ORDER BY
                                ...
                                (  SELECT   po_header_id,
                                            ...
                                            SUM (quantity_rejected)
                                               qty_rejected,
                                            SUM (quantity_billed) qty_billed,
                                            SUM (quantity_cancelled)
                                               qty_cancelled,
                                            MIN (need_by_date) need_by_date
                                     FROM   --po.po_line_locations_all -- Commented by IT Convergence as Per R12.2.10 Upgrade
									 po_line_locations_all -- Added by IT Convergence as Per R12.2.10 Upgrade
                                ...
                                (SELECT   *
                                   FROM   (SELECT   hp.party_id,
                                            ...
                                            WHERE   hp.party_id =
                                                    ...
                                                    AND hr.object_id =
                                                          hp_per.party_id)
                                  WHERE   last_created_cont = 1) party_cont,
                                (  SELECT   apila.po_line_id,
                                            apila.po_header_id,
                                            apila.invoice_id ap_invoice_id,
                                            apia.invoice_num ap_inv_number,
                                            SUM (apipa.amount)
                                               ap_inv_payment_amount,
                                            MAX (apipa.accounting_date)
                                               ap_inv_last_payment_date
                                     FROM   ap_invoice_lines_all apila,
                                            ap_invoices_all apia,
                                            ap_invoice_payments_all apipa
                                    WHERE   apila.invoice_id = apia.invoice_id
                                            AND NVL (apila.discarded_flag, 'N') <>
                                                  'Y'
                                            AND apila.invoice_id =
                                                  apipa.invoice_id(+)
                                 /* AND apila.po_header_id IN (SELECT po_header_id FROM po_headers_all WHERE segment1 IN ('40410','41355','40017','40019')) */
                                 GROUP BY   apila.po_line_id,
                                            apila.po_header_id,
                                            apila.invoice_id,
                                            apia.invoice_num) ap_inv
                        WHERE   pha.po_header_id = pla.po_header_id
                                AND pha.vendor_id = aps.vendor_id
                                AND pha.vendor_site_id =
                                      apssa.vendor_site_id(+)
                                AND pha.po_header_id = po_act.object_id(+)
                                AND pha.revision_num =
                                      po_act.object_revision_num(+)
                                AND pla.po_line_id = po_rcpt.po_line_id(+)
                                AND pha.revision_num =
                                      po_rcpt.po_revision_num(+)
                                /* AND xpbt.po_number IN (41500,41506) (41535, 41502) */
                                AND aps.party_id = party_cont.party_id(+)
                                AND pla.po_line_id = ap_inv.po_line_id(+)
                                AND pla.po_line_id = pda.po_line_id(+)
                                AND pda.code_combination_id =
                                      gcc.code_combination_id(+)
                                AND pla.po_header_id =
                                      po_ln_loc.po_header_id(+)
                                AND pla.po_line_id = po_ln_loc.po_line_id(+)
                                AND pha.po_header_id = xpbt.po_header_id(+)
                                AND pla.po_line_id = xpbt.po_line_id(+)
                                AND xpbt.po_header_id(+) > 0
                                AND xpbt.link_to_order_line_id(+) IS NOT NULL /* AND xpbt.po_cancel_ok = 'N' */
                                                                             ) v
