CREATE OR REPLACE FORCE VIEW EIS_OM_ORDER_DETAILS_XXMGRC_V4
(
   ORG_ID,
   ORDER_TYPE_ID,
   ORDER_NUMBER,
...
AS
   SELECT --In Addition to std. EIS table joins, this view has joins to following tables:
      ...
      --Custom OM Billing TRX table which is using for recurring billing only.
            ...
            ROW_NUMBER ()
               OVER (PARTITION BY ol.line_id
                     ORDER BY ombilltrx.om_billing_trx_id ASC)
               ombilltrx_row_num,                     /* MGRC customization */
     ...
     FROM   oe_order_headers oh,
		 ...
		 oe_transaction_types_vl oltt,  /* MGRC customization */ -- Added by IT Convergence as Per R12.2.10 Upgrade
           ...
           -- bolinf.xxmgrc_om_billing_trx ombilltrx    /* MGRC customization */ -- Commented by IT Convergence as Per R12.2.10 Upgrade
		   xxmgrc_om_billing_trx ombilltrx  /* MGRC customization */  -- Added by IT Convergence as Per R12.2.10 Upgrade
    WHERE       ol.header_id = oh.header_id
            ...
            AND ombill.oe_line_id = ombilltrx.oe_line_id(+) /* MGRC customization */;
/
SHOW ERROR
EXIT;
/
